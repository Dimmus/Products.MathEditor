...
jsMath.Add(jsMath.HTML,{
  ...
  BBox: function (w,h,d,c,s) {
	//The argument "c" contains the @id
	//The argument "s" is no longer used (could use it to represent the cursor)
    ...
    style+="border:1px green dotted";
    var html = '<span id="'+c+'" title="'+c+'" class="blank" style="'+style+'"></span>';
...
jsMath.Add(jsMath.mList.prototype.Atomize,{
   ...
   */
  matheditor: function (style,size,mitem,prev,mlist) {
...
jsMath.Package(jsMath.Parser,{
  ...
  BBox: function (name) {
    ...
    this.mlist.Add(new jsMath.mItem('matheditor',atom));
  }
});
