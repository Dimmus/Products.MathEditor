var firebug = {
    version:[1.21,20081222],
    el:{}, env:{ "cache":{}, "override":false, "css":"../lib/firebug-lite/firebug-lite.css", "dIndex":"console", "height":295, "init":false, "minimized":false, "ml":false },
    init:function(_css){
        with(firebug){
            ...
            document.getElementsByTagName("head")[0].appendChild(
                new lib.element("link").attribute.set("rel","stylesheet").attribute.set("href",firebug.env.css||"../lib/firebug-lite/firebug-lite.css").element
            );
            ...
            el.main.child.add(new lib.element("DIV").environment.addStyle({ "clear":"both" }));
            ...
            el.button.inspect = new lib.element("A").attribute.addClass("Button").event.addListener("click",d.inspector.toggle).update("Inspect").insert(el.button.container);
            ...
            el.nav.dom = new lib.element("A").attribute.addClass("Tab").update("DOM").event.addListener("click",lib.util.Curry(d.navigate,window,"dom")).insert(el.nav.container);
            ...
            el.button.dom.textbox = new lib.element("INPUT").event.addListener("keydown",listen.domTextbox).update("window").insert(el.button.dom.container);
    ...
    win:{
        close:function(){
            with(firebug){
                el.main.update("");
                el.main.remove();
                lib.util.RemoveEvent(window,"resize",win.refreshSize);
                lib.util.RemoveEvent(document,"mousemove",listen.mouse)("keydown",firebug.listen.keyboard);
                env.init = false;
            }
        },
        minimize:function(){
    ...
    d: {
            ...
            eval:function(_cmd){
                with(firebug){
                    ...
                    try { 
                        ...
                        d.console.addLine().attribute.addClass("Arrow").update(">>> "+_cmd);
                        if(!lib.env.ff)
                            d.console.addLine().attribute.addClass("Error").update("<strong>Error: </strong>"+(e.description||e),true);
                        else {
                            if(e.fileName==null){
                ...
                with (firebug) {
                    ...
                    for (var key in obj) {
                        try { 
                            ...
                            container.child.add(
                                new lib.element("DIV").environment.addStyle({ "clear":"both" })
                            );
        ...
        highlight:function(_value,_inObject,_inArray,_link){
            with(firebug){
                ...
                // array & object
                else if(isArray||lib.util.Array.indexOf(["object","array"],typeof _value)>-1){
        ...
        html:{
            ...
            inspect:function(_element){
                ...
                with(firebug){
                    ...
                    var parentLayer = el.left.html.container.child.get()[4].childNodes[1].lib;
            ...
            openHtmlTree:function(_element,_parent,_returnParentElementByElement,_event){
                with(firebug){
                    var
                    element = _element || lib.get.byTag("html")[0], 
                    ...
                    if(parent!=el.left.html.container){
                        var nodeLink = parent.environment.getParent().lib.child.get()[0].lib;
                        if (d.html.current) {
                            d.html.current[1].attribute.removeClass("Selected");
                        }
        ...
        inspector:{
            ...
            inspect:function(_element,_bgInspector){
                with(firebug){
                    var pos = lib.util.Element.getPosition(_element);
                ...
                with(firebug){
                    ...
                    if (!d.scripts.lineNumbers) 
                        ...
                        for (var i = 0; i < source.length; i++) {
                            el.left.scripts.container.child.add(new lib.element("DIV").child.add(new lib.element("DIV").attribute.addClass("LineNumber").update(i + 1), new lib.element("DIV").attribute.addClass("Code").update("&nbsp;" + source[i]), new lib.element("DIV").environment.addStyle({
                                "clear": "both"
                            })));
                        };
                    };
                ...
                with(firebug){
                    ...
                    for(var i=0,len=collection.length; i<len; i++){
                        ...
                        el.button.scripts.selectbox.child.add(
                            new lib.element("OPTION").attribute.set("value",i).update(item.src||item.baseURI||"..")
                        );
                    }
                    d.scripts.open( d.scripts.index );
                }
    ...
    listen: {
        ...
        consoleTextbox:function(_event){
            with(firebug){
                ...
                switch(_event.keyCode){
                    case 38:
                        d.console.historyIndex+=Number(d.console.historyIndex!=d.console.history.length);
                        update = true;
                        break;
                    case 40:
                        d.console.historyIndex-=Number(d.console.historyIndex!=0);
                        update = true;
                        break;
                };
