...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="math-disarray">
    ...
    <xsl:text>\typeout{Textwidth = \the\textwidth}
    </xsl:text>
    <xsl:text>\typeout{math as usual width = \the\mymathboxwidth}
    </xsl:text>

    <!-- Test the width -->
    <xsl:text>\ifthenelse{\lengthtest{\mymathboxwidth &lt; \textwidth}}{</xsl:text>
    ...
    <!-- Typeset this math in the de-arrayed fashion; the LaTeX output of 
         ...
         above was too wide. -->
    <xsl:apply-templates select="self::*" mode="math-disarray"/>
